# HTML Learned By Code With Harry

## Overview

This folder contains HTML code and projects that I learned through **Code With Harry's YouTube Tutorials**, PDF notes, websites, and other learning resources. It showcases my journey from beginner to advanced HTML concepts.

## Learning Path

### Beginner Level

- HTML Fundamentals
- HTML Tags and Elements
- Document Structure
- Text Formatting
- Links and Navigation
- Images and Media
- Lists and Tables
- Forms and Input Elements

### Intermediate Level

- Semantic HTML
- Accessibility Features
- SEO Best Practices
- Meta Tags and Head Elements
- Data Attributes
- HTML5 Features
- Canvas and SVG Basics

### Advanced Level

- Complex Form Validation
- Web Components Concepts
- Progressive Enhancement
- HTML with Web APIs
- Responsive HTML Structure
- Performance Optimization

## Learning Resources

- 📺 **Code With Harry** - YouTube Tutorials
- 📄 **PDF Notes** - Study Materials
- 🌐 **Websites** - Reference Documentation
- 💻 **Hands-on Practice** - Project-based Learning

## Folder Structure

This folder contains various HTML projects and exercises demonstrating:

- Proper HTML structure
- Semantic HTML elements
- Form handling
- Media embedding
- Accessibility features
- Real-world applications

## How to Use This Folder

1. Browse through the files to find different HTML topics
2. Each file contains HTML code with examples and explanations
3. Open the HTML files in your browser to see them in action
4. Review the code comments for detailed explanations
5. Modify and experiment with the code to learn better

## Topics Covered

- Document structure and boilerplate
- HTML elements and tags
- Forms and user input
- Multimedia (images, audio, video)
- Tables and data representation
- Navigation and hyperlinks
- Semantic HTML5 elements
- Accessibility attributes (ARIA, alt text, etc.)
- Meta information and SEO
- Embedded content (iframes, embeds)

## Getting Started

1. Clone this repository
   ```bash
   git clone https://github.com/hassanbinzubair/CSS_Learned_By_Code_With_Harry.git
   ```

2. Navigate to the HTML folder
   ```bash
   cd CSS_Learned_By_Code_With_Harry/HTML
   ```

3. Open any HTML file in your browser to see the examples

## Future Goals

- Build more complex interactive HTML projects
- Improve accessibility and semantic markup
- Implement advanced form features
- Explore HTML5 APIs

---

**Repository**: [CSS_Learned_By_Code_With_Harry](https://github.com/hassanbinzubair/CSS_Learned_By_Code_With_Harry)

**Language Composition of Main Repository**:
- HTML: 79.2%
- CSS: 20.8%
